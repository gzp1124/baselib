...
class MyHttpLoggingInterceptor @JvmOverloads constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        var requestStartMessage =
            ("req start--> ${request.method} ${request.url}${if (connection != null) " " + connection.protocol() else ""}")
...
//            }
        ...
        logger.log(
            "req end<-- ${response.code}${if (response.message.isEmpty()) "" else ' ' + response.message} ${response.request.url} (${tookMs}ms${if (!logHeaders) ", $bodySize body" else ""})")
...
//        logger.log(headers.name(i) + ": " + value)
        ...
        for (i in 0 until headers.size) {
            val value = if (headers.name(i) in headersToRedact) "██" else headers.value(i)
            hs.append(headers.name(i) + ": " + value+" \n")
        }
