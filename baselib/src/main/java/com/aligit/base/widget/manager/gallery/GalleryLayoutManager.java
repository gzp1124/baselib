...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (state.getItemCount() != 0 && !state.didStructureChange()) {
            if (BuildConfig.DEBUG) {
                XLog.d("onLayoutChildren: ignore extra layout step");
            }
            return;
        }
    ...
    private void firstFillCover(RecyclerView.Recycler recycler, RecyclerView.State state, int scrollDelta) {
        ...
        if (BuildConfig.DEBUG) {
            XLog.d("firstFillCover finish:first: " + mFirstVisiblePosition + ",last:" + mLastVisiblePos);
        }
    ...
    private float calculateToCenterFraction(View child, float pendingOffset) {
        ...
        if (BuildConfig.DEBUG) {
            XLog.d("calculateToCenterFraction: distance:" + distance + ",childLength:" + childLength);
        }
    ...
    private void fillWithVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int dy) {
            ...
            if (dy >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedBottom(child) - dy < topEdge) {
                        if (BuildConfig.DEBUG) {
                            XLog.v("fillWithVertical: removeAndRecycleView:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        }
                        ...
                        if (BuildConfig.DEBUG) {
                            XLog.d("fillWithVertical: break:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        }
                        break;
                    }
                }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedTop(child) - dy > bottomEdge) {
                        if (BuildConfig.DEBUG) {
                            XLog.v("fillWithVertical: removeAndRecycleView:" + getPosition(child));
                        }
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
                    } else {
        ...
        if (dy >= 0) {
            ...
            for (int i = startPosition; i < getItemCount() && startOffset < bottomEdge + dy; i++) {
                ...
                mLastVisiblePos = i;
                if (BuildConfig.DEBUG) {
                    XLog.d("fillWithVertical: add view:" + i + ",startOffset:" + startOffset + ",mLastVisiblePos:" + mLastVisiblePos + ",bottomEdge" + bottomEdge);
                }
            }
        } else {
            //dy<0
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        int rightEdge = getOrientationHelper().getEndAfterPadding();
        if (BuildConfig.DEBUG) {
            XLog.v("fillWithHorizontal() called with: dx = [" + dx + "],leftEdge:" + leftEdge + ",rightEdge:" + rightEdge);
        }
        //1.remove and recycle the view that disappear in screen
        View child;
        if (getChildCount() > 0) {
            if (dx >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedRight(child) - dx < leftEdge) {
                        ...
                        fixIndex--;
                        if (BuildConfig.DEBUG) {
                            XLog.v("fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + " mFirstVisiblePosition change to:" + mFirstVisiblePosition);
                        }
                    } else {
                        break;
                    }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedLeft(child) - dx > rightEdge) {
                        ...
                        mLastVisiblePos--;
                        if (BuildConfig.DEBUG) {
                            XLog.v("fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + "mLastVisiblePos change to:" + mLastVisiblePos);
                        }
                    }
                }
            }
        ...
        if (dx >= 0) {
            if (getChildCount() != 0) {
                ...
                if (BuildConfig.DEBUG) {
                    XLog.d("fillWithHorizontal:to right startPosition:" + startPosition + ",startOffset:" + startOffset + ",rightEdge:" + rightEdge);
                }
            }
            for (int i = startPosition; i < getItemCount() && startOffset < rightEdge + dx; i++) {
                ...
                mLastVisiblePos = i;
                if (BuildConfig.DEBUG) {
                    XLog.d("fillWithHorizontal,layout:mLastVisiblePos: " + mLastVisiblePos);
                }
            }
        } else {
            //dx<0
            if (getChildCount() > 0) {
                ...
                if (BuildConfig.DEBUG) {
                    XLog.d("fillWithHorizontal:to left startPosition:" + startPosition + ",startOffset:" + startOffset + ",leftEdge:" + leftEdge + ",child count:" + getChildCount());
                }
            }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (BuildConfig.DEBUG) {
            XLog.d("scrollHorizontallyBy: dx:" + dx + ",fixed:" + delta);
        }
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (BuildConfig.DEBUG) {
            XLog.d("scrollVerticallyBy: dy:" + dy + ",fixed:" + delta);
        }
        getState().mScrollDelta = -delta;
        fillCover(recycler, state, -delta);
    ...
    private class InnerScrollListener extends RecyclerView.OnScrollListener {
            ...
            if (snap != null) {
                ...
                if (selectedPosition != mCurSelectedPosition) {
                    ...
                    if (!mCallbackInFling && mState != SCROLL_STATE_IDLE) {
                        if (BuildConfig.DEBUG) {
                            XLog.v("ignore selection change callback when fling ");
                        }
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            mState = newState;
            if (BuildConfig.DEBUG) {
                XLog.v("onScrollStateChanged: " + newState);
            }
            if (mState == SCROLL_STATE_IDLE) {
                ...
                if (snap != null) {
                    ...
                    XLog.e("onScrollStateChanged: snap null");
                }
            }
        }
    }
