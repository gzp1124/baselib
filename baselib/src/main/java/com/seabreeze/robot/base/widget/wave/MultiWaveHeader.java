package com.seabreeze.robot.base.widget.wave;
...
public class MultiWaveHeader extends ViewGroup {
    ...
    public MultiWaveHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mWaveHeight = ta.getDimensionPixelOffset(R.styleable.MultiWaveHeader_mwhWaveHeight, dp2px(context, 50));
        ...
        mCornerRadius = ta.getDimensionPixelOffset(R.styleable.MultiWaveHeader_mwhCornerRadius, dp2px(context, 25));
...
//    }
    ...
    protected void updateWavePath() {
        ...
        if (getTag() instanceof String) {
            ...
            for (String wave : waves) {
                ...
                if (args.length == 5) {
                    mltWave.add(new Wave(mContext, dp2px(mContext, parseFloat(args[0])), dp2px(mContext, parseFloat(args[1])),
                            dp2px(mContext, parseFloat(args[4])), parseFloat(args[2]), parseFloat(args[3]), mWaveHeight / 2));
                }
            }
        } else {
            mltWave.add(new Wave(mContext, dp2px(mContext, 50), dp2px(mContext, 0), dp2px(mContext, 5), 1.7f, 2f, mWaveHeight / 2));
        }
    }
    ...
    public void setWaveHeight(int waveHeight) {
        this.mWaveHeight = dp2px(mContext, waveHeight);
        if (!mltWave.isEmpty()) {
            View thisView = this;
            updateWavePath(thisView.getWidth(), thisView.getHeight());
        }
